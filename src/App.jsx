    ...
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.82)",backdropFilter:"blur(8px)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:16,animation:"fadeIn 0.18s ease"}}>
      <div style={{background:"#111113",border:"1px solid #252528",borderRadius:18,padding:28,width:"100%",maxWidth,maxHeight:"92vh",overflowY:"auto",animation:"scaleIn 0.22s cubic-bezier(0.34,1.56,0.64,1)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:22}}>
          ...
          <button onClick={onClose} style={{background:"none",border:"1px solid #252528",color:"#6b6b72",width:30,height:30,borderRadius:8,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.15s"}}>{Icon.close}</button>
        </div>
        {children}
      </div>
    </div>
    ...
    <div style={{minHeight:"100vh",background:"#0c0c0e",display:"flex",alignItems:"center",justifyContent:"center",padding:20,position:"relative",overflow:"hidden"}}>
      ...
      <div style={{width:"100%",maxWidth:400,animation:"fadeUp 0.55s ease"}}>
        ...
        <div style={{textAlign:"center",marginBottom:32}}>
          ...
          <div style={{fontWeight:800,fontSize:"1.55rem",letterSpacing:"-0.04em"}}>
          ...
          <form onSubmit={handle}>
            <Field label="Email">
              <div style={{position:"relative"}}>
                ...
                <input
                  ...
                  style={{...INP,paddingLeft:38,borderColor:focusedField==="email"?"#e8673a":"#2a2a2e"}}
                />
              </div>
            </Field>
            ...
            <Field label="Password">
              <div style={{position:"relative"}}>
                ...
                <input
                  type={showPw?"text":"password"} value={pw} required
                  onChange={e=>setPw(e.target.value)}
                  onFocus={()=>setFocusedField("pw")}
                  onBlur={()=>setFocusedField("")}
                  placeholder="Enter your password"
                  style={{...INP,paddingLeft:38,paddingRight:42,borderColor:focusedField==="pw"?"#e8673a":"#2a2a2e"}}
                />
                <button type="button" onClick={()=>setShowPw(s=>!s)} style={{position:"absolute",right:11,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:"#4a4a55",cursor:"pointer",display:"flex",padding:3,transition:"color 0.15s"}}>
            ...
            <button type="submit" disabled={loading} style={{width:"100%",padding:"12px",background:loading?"#252528":"linear-gradient(135deg,#e8673a,#c4512a)",color:loading?"#6b6b72":"#fff",border:"none",borderRadius:10,fontSize:"0.88rem",fontWeight:700,cursor:loading?"not-allowed":"pointer",transition:"all 0.2s",marginTop:4,display:"flex",alignItems:"center",justifyContent:"center",gap:9,letterSpacing:"0.02em"}}>
              {loading?(
                <><span style={{width:15,height:15,border:"2px solid rgba(255,255,255,0.25)",borderTopColor:"#fff",borderRadius:"50%",display:"inline-block",animation:"spin 0.7s linear infinite"}}/> Signing in...</>
...
export default function App(){
  ...
  const [page,setPage]=useState("dashboard"); // dashboard | project | client
  ...
  /* Copy share link — opens client view in same tab */
  function copyShareLink(){
    const p=getP(activeProjectId);
    const shareText=`${window.location.href.split("?")[0]}?view=client&project=${p.id}`;
      ...
      <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
        ...
        <div style={{padding:"20px 16px 16px",borderBottom:"1px solid #1e1e21"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:34,height:34,background:"linear-gradient(135deg,#e8673a,#c4512a)",borderRadius:10,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",flexShrink:0,boxShadow:"0 4px 14px rgba(232,103,58,0.3)"}}>
              {Icon.pulse}
            </div>
            <div>
        ...
        <div style={{padding:"8px 10px",flex:1,overflowY:"auto"}}>
          <div style={{fontSize:"0.62rem",letterSpacing:"0.1em",textTransform:"uppercase",color:"#3a3a42",padding:"0 8px",marginBottom:6,fontWeight:700}}>
            Projects <span style={{color:"#4a4a55"}}>{projects.length}</span>
          </div>
          {projects.length===0&&(
            <div style={{fontSize:"0.78rem",color:"#4a4a55",padding:"6px 10px"}}>No projects yet</div>
          )}
          {projects.map((p,i)=>(
  ...
  return(
    <>
        ...
        <div style={{flex:1,overflowX:"hidden",minWidth:0}}>
              ...
              <div style={{padding:"22px 20px"}}>
                ...
                <div className="stats-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:26}}>
                  {[
                    ...
                    <div key={s.label} style={{background:"#111113",border:"1px solid #1e1e21",borderRadius:14,padding:"18px 16px",animation:`fadeUp 0.45s ease ${i*0.07}s both`,transition:"border-color 0.2s"}}>
                ...
                {projects.some(p=>p.updates.length>0)&&(
                  <div style={{marginBottom:26}}>
                    <div style={{fontWeight:700,fontSize:"0.82rem",marginBottom:12,color:"#6b6b72",letterSpacing:"0.04em",textTransform:"uppercase",display:"flex",alignItems:"center",gap:7}}>
                ...
                <div className="detail-grid" style={{display:"grid",gridTemplateColumns:"1fr 288px",gap:18,alignItems:"start"}}>
                  ...
                  <div style={{background:"#111113",border:"1px solid #1e1e21",borderRadius:15,overflow:"hidden"}}>
                    <div style={{padding:"15px 18px",borderBottom:"1px solid #1e1e21",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                      <div style={{fontWeight:700,fontSize:"0.9rem"}}>
                        Updates
                        <span style={{fontSize:"0.72rem",fontWeight:400,color:"#4a4a55",marginLeft:8}}>{activeProject.updates.length} total</span>
                      </div>
                    ...
                    <div style={{padding:8}}>
                      {activeProject.updates.length===0?(
                        ...
                        [...activeProject.updates].reverse().map((u,ri)=>(
                          <div key={u.id} style={{padding:"13px 14px",borderRadius:11,marginBottom:3,transition:"background 0.15s",animation:`fadeUp 0.3s ease ${ri*0.04}s both`,background:"transparent"}}>
                            <div style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:8,flexWrap:"wrap"}}>
                              ...
                              <div style={{display:"flex",gap:4,flexShrink:0}}>
                                <button onClick={()=>openEditUpdate(u.id)} title="Edit" style={{width:28,height:28,display:"flex",alignItems:"center",justifyContent:"center",borderRadius:7,border:"1px solid #1e1e21",background:"transparent",color:"#6b6b72",cursor:"pointer",transition:"all 0.15s"}}>{Icon.edit}</button>
                                <button onClick={()=>setConfirm({title:"Delete Update",msg:`Delete "${u.title}"? This cannot be undone.`,onOk:()=>deleteUpdate(u.id)})} title="Delete" style={{width:28,height:28,display:"flex",alignItems:"center",justifyContent:"center",borderRadius:7,border:"1px solid rgba(232,85,85,0.18)",background:"rgba(232,85,85,0.06)",color:"#e85555",cursor:"pointer",transition:"all 0.15s"}}>{Icon.trash}</button>
                              </div>
                            </div>
                            ...
                            <div style={{display:"flex",alignItems:"center",gap:6,marginTop:9,flexWrap:"wrap"}}>
                              ...
                              {u.tags.map(t=>(
                                <span key={t} style={{fontSize:"0.65rem",padding:"2px 7px",background:"#1a1a1d",border:"1px solid #1e1e21",borderRadius:4,color:"#6b6b72",display:"flex",alignItems:"center",gap:3}}>
                                  {Icon.tag} {t}
                                </span>
                              ))}
                              {u.link&&(
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:14}}>
                    ...
                    <div style={{background:"rgba(232,103,58,0.05)",border:"1px solid rgba(232,103,58,0.15)",borderRadius:14,padding:16}}>
                      <div style={{fontSize:"0.72rem",fontWeight:700,color:"#e8673a",marginBottom:10,letterSpacing:"0.05em",textTransform:"uppercase",display:"flex",alignItems:"center",gap:6}}>{Icon.link} Client Share Link</div>
                      <div style={{fontSize:"0.71rem",color:"#6b6b72",marginBottom:10,lineHeight:1.5}}>Share this link with your client so they can view project updates without logging in.</div>
                      <div style={{display:"flex",gap:8,alignItems:"center",background:"#0c0c0e",border:"1px solid #1e1e21",borderRadius:9,padding:"8px 10px"}}>
                        <div style={{flex:1,fontSize:"0.69rem",color:"#6b6b72",fontFamily:"monospace",wordBreak:"break-all"}}>projectpulse.app/p/{slugify(activeProject.name)}</div>
                      ...
                      <button onClick={()=>setPage("client")} style={{width:"100%",marginTop:10,padding:"8px",background:"transparent",border:"1px solid rgba(232,103,58,0.2)",borderRadius:8,color:"#e8673a",fontSize:"0.77rem",fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:6,transition:"all 0.2s"}}>
                    ...
                    <div style={{background:"rgba(232,85,85,0.04)",border:"1px solid rgba(232,85,85,0.15)",borderRadius:14,padding:16}}>
                      ...
                      <div style={{fontSize:"0.78rem",color:"#6b6b72",marginBottom:12,lineHeight:1.5}}>Permanently delete this project and all its updates. This cannot be undone.</div>
                      <Btn variant="danger" size="sm" onClick={()=>setConfirm({title:"Delete Project",msg:`Are you sure you want to delete "${activeProject.name}"? All updates will be permanently removed.`,onOk:()=>deleteProject(activeProjectId)})}>
          ...
          {page==="client"&&activeProject&&(
            <div style={{animation:"fadeIn 0.4s ease"}}>
              <Topbar title="Client View" subtitle="This is what your client sees" onMenu={()=>setSidebarOpen(true)}>
              ...
              <div style={{background:"#111113",borderBottom:"1px solid #1e1e21",padding:"44px 24px 34px",position:"relative",overflow:"hidden"}}>
                <div style={{position:"absolute",top:-100,right:-60,width:340,height:340,borderRadius:"50%",background:`radial-gradient(circle,${activeProject.color}18,transparent 68%)`,pointerEvents:"none"}}/>
                ...
                <div style={{maxWidth:640,margin:"0 auto",position:"relative"}}>
                  <div style={{display:"inline-flex",alignItems:"center",gap:7,fontSize:"0.67rem",fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",color:activeProject.color,background:`${activeProject.color}16`,border:`1px solid ${activeProject.color}30`,padding:"4px 12px",borderRadius:20,marginBottom:18,animation:"fadeUp 0.45s ease"}}>
                    <span style={{width:6,height:6,borderRadius:"50%",background:activeProject.color,display:"inline-block",animation:"pulseDot 2s ease infinite"}}/>
                    Live Updates
                  </div>
                  <h1 style={{fontWeight:800,fontSize:"clamp(1.6rem,4vw,2.5rem)",letterSpacing:"-0.04em",marginBottom:10,lineHeight:1.1,animation:"fadeUp 0.45s ease 0.05s both"}}>{activeProject.name}</h1>
                  <p style={{fontSize:"0.91rem",color:"#9b9ba3",maxWidth:420,lineHeight:1.68,marginBottom:26,animation:"fadeUp 0.45s ease 0.1s both"}}>{activeProject.desc||""}</p>
                  <div style={{display:"flex",gap:22,flexWrap:"wrap",animation:"fadeUp 0.45s ease 0.15s both"}}>
                    {[["Client",activeProject.client,Icon.user],["Status",projStatusLabel(activeProject.status),Icon.activity],["Started",fmtDate(activeProject.start),Icon.calendar],["Deadline",fmtDate(activeProject.deadline),Icon.calendar]].map(([l,v,ic])=>(
                      <div key={l}>
              ...
              <div style={{maxWidth:640,margin:"0 auto",padding:"30px 20px 60px"}}>
                <div style={{fontWeight:700,fontSize:"0.82rem",marginBottom:20,display:"flex",alignItems:"center",gap:10,color:"#6b6b72",letterSpacing:"0.04em",textTransform:"uppercase"}}>
                  ...
                  <span style={{fontWeight:400,color:"#3a3a42"}}>{activeProject.updates.length} total</span>
                </div>
                {activeProject.updates.length===0?(
                  ...
                  [...activeProject.updates].reverse().map((u,i,arr)=>{
                    ...
                    return(
                      <div key={u.id} style={{display:"grid",gridTemplateColumns:"28px 1fr",gap:"0 14px",animation:`fadeUp 0.45s ease ${i*0.07}s both`}}>
                        ...
                        <div>
                          <div style={{background:"#111113",border:"1px solid #1e1e21",borderRadius:13,padding:"15px 18px",marginBottom:11,marginTop:10,transition:"border-color 0.2s"}}>
      ...
      <Modal open={projModal} onClose={()=>setProjModal(false)} title={editProjId?"Edit Project":"New Project"}>
        ...
        <Field label="Client Email" hint="Used for the share link reference"><input style={INP} type="email" value={pm.email} onChange={e=>setPm(p=>({...p,email:e.target.value}))} placeholder="client@example.com"/></Field>
          ...
          <div style={{display:"flex",flexWrap:"wrap",gap:6,padding:"7px 8px",border:"1px solid #2a2a2e",borderRadius:9,background:"#1a1a1d",cursor:"text",minHeight:44,alignItems:"center"}}>
            {um.tags.map((t,i)=>(
              <span key={i} style={{display:"inline-flex",alignItems:"center",gap:4,background:"#252528",border:"1px solid #2a2a2e",borderRadius:5,padding:"2px 8px",fontSize:"0.72rem",color:"#9b9ba3"}}>
                {t}
                <button onClick={()=>removeTag(i)} style={{background:"none",border:"none",color:"#4a4a55",cursor:"pointer",display:"flex",alignItems:"center",padding:0,transition:"color 0.15s"}}>{Icon.x}</button>
              </span>
            ))}
